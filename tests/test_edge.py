...
class TestVoid(unittest.TestCase):
    ...
    def test_len01(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(0.0, 0.0, 0.0))
        self.assertAlmostEqual(s.__len__(), 0)

    def test_len02(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(1.0, 0.0, 0.0))
        self.assertAlmostEqual(s.__len__(), 1)

    def test_len03(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(0.0, 3.0, -4.0))
        self.assertAlmostEqual(s.__len__(), 5)

    def test_center01(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(1.0, 0.0, 0.0))
        self.assertIsInstance(s.center(), R3)

    def test_center02(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(0.0, 0.0, 0.0))
        self.assertEqual(R3ApproxMatcher(s.center()), R3(0.0, 0.0, 0.0))

    def test_center03(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(2.0, 6.0, -4.0))
        self.assertEqual(R3ApproxMatcher(s.center()), R3(1.0, 3.0, -2.0))

    def test_angle_with_vector01(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(1.0, 0.0, 0.0))
        V = R3(1.0, 0.0, 0.0)
        self.assertAlmostEqual(s.angle_with_vector(V), 0)

    def test_angle_with_vector02(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(1.0, 0.0, 0.0))
        V = R3(0.5, sqrt(3) / 2, 0.0)
        self.assertAlmostEqual(s.angle_with_vector(V), pi / 3)

    def test_angle_with_vector03(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(0.0, 2.0, 0.0))
        V = R3(1.0, 0.0, 0.0)
        self.assertAlmostEqual(s.angle_with_vector(V), pi / 2)
