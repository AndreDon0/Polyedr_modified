...
class Edge:
    ...
    def shadow(self, facet):
        # Не надо ничего делать, если «просветов» на ребре не осталось
        if len(self.gaps) == 0:
            return
    ...
    # Длинна ребра (для задания №64)
    def __len__(self):
        length = 0
        for s in self.gaps:
            length += (self.r3(s.fin) - self.r3(s.beg)).length()
        return length

    # Центр ребра (для задания №64)
    def center(self):
        center = self.r3((self.SFIN - self.SBEG) / 2)  # Центр ребра
        return center
